...
class TechDocsPage {
    constructor(page) {
      ...
      this.searchBarInput = page.locator('input.select2-search__field[role="textbox"]')
    ...
    async open() {
      ...
      const pageHeader = await this.page.locator('h1.page-header-title').textContent();
      return  pageHeader 
    }
    ...
    async filterProduct(prodcutName, productGroupName, lang, docNumber) {
      ...
      await this.page.waitForTimeout(1000)
      await this.page.locator(`li[role="treeitem"]:has-text("${docNumber}")`).click();
...
async downloadDocument() {
  ...
  await download.saveAs('./downloads/document.pdf'); // Adjust the path if needed
